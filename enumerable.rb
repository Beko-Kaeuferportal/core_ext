  ...
  def symbolize_keys
    if is_a?(Hash)
      inject({}) do |options, (key, value)|
        options[(key.to_sym rescue key) || key] = (value.is_a?(Hash)||value.is_a?(Array)) ? value.symbolize_keys : value
        options
      end
    elsif is_a?(Array)
      inject([]) do |options, value|
        options << ((value.is_a?(Hash)||value.is_a?(Array)) ? value.symbolize_keys : value)
        options
      end
    else
